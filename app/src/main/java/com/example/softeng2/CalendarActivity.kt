            ...
            .addOnSuccessListener { querySnapshot ->
                ...
                count = querySnapshot.size()
                    for (document in querySnapshot.documents) {
                        ...
                        map.put(Pair(timeSlotsStr.indexOf(document.data!!.get("Time").toString())+2,ChronoUnit.DAYS.between(curDateBasis,test).toInt()+1),1)
                    ...
                    for (col in 0 until gridLayout.columnCount) {
                        val textView = TextView(this).apply {
                            ...
                            if (row == 0) {
                                ...
                                data["col"]=col
                                data["row"]=row
                                data["date"]= LocalDate.of(now.year, now.monthValue, now.dayOfMonth).plusDays((col-1).toLong())
                                tag=data
                                setOnClickListener {
                                    if (res==1) {
                                        Toast.makeText(applicationContext, "This slot is unavailable", Toast.LENGTH_SHORT).show()
                                    } else {
                                        ...
                                        retrievedVal?.let {
                                            ...
                                            val time = "${
                                                timeSlots[row.toString().toInt()-2] / 60
                                            }:${
